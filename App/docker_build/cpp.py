...
def build(filename):
    ...
    docker_contalner = str(client.containers.create(data['cpp']).id) # 컨테이너 종류 / 컨테이너 실행
    ...
    return {"test build language" : "cpp/gcc", "container build time" : round(build_end - build_start, 3), "file execution time" : round(run_end - run_start, 3), "file result" : return_data}