...
def build(filename):
    ...
    os.system(f"docker cp Data/user_file/{filename} {docker_contalner}:home/run.go") # 컨테이너에 파일 전송
    ...
    return {"test build language" : "golang", "container build time" : round(build_end - build_start, 3), "file execution time" : round(run_end - run_start, 3), "file result" : return_data}